...
\noindent\PrimaryColor{\spacedlowsmallcaps{Goal 4 -- Applications for biological systems.}}\hspace{3pt} Last, but not least, since the topic of Boolean network control is tightly tied to the cell reprogramming, we demonstrate that the developed methods are serving the original motivation goal. Later, we realized, that control can serve also as a tool for model refinement, thus ultimately solving the original problem of cell reprogramming more precisely. Both these applications and case studies illustrating their utility are presented in \autoref{chapter:applications}.
