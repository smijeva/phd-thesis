...
In this chapter, we first define the source-target control problem in \autoref{s:source_target_problem_definition}, including the notions of one-step, permanent, and temporary perturbations. Next, in \autoref{sec:target_semisymbolic}, we present a semi-symbolic method for computing one-step source-target control in partially specified Boolean networks. Finally, in \autoref{sec:ostp_symbolic}, we extend this method by representing the state space of perturbed networks fully symbolically and to handle permanent and temporary perturbations as well.
...
\subsection{Perturbations, perturbed state transition graph and runs}
...
\begin{definition}
    ...
    \run_{\perturbation}(t) = \begin{cases}
        ...
        s'  \text{ s. t. } \run(t-1) = s'' \land (s'', s') \in \edges_{\perturbation} & \text{iff }  t'+z \geq t > t' + 1 \\
...
In practice, our goal is to not only find perturbations that successfully control the network, but that are efficient and biologically feasible. Indeed, for a target state $t$ a trivial perturbation $\perturbation = t$ successfully controls the network towards $t$, but is most likely not realistic. When the network is fully known, we typically focus on finding perturbations that are minimal in terms of size~\cite{Su2019CMSB,Su2020}. This search can be further restricted to network variables where the perturbation is known to be biologically feasible if such information is available.
...
\begin{align*}
	\rho_{\max}(\controlRelation_{X}) &= \max_{\perturbation \in \allPerturbations} \rho(\perturbation) \\
...
\begin{algorithm}
  \SetKwProg{Fn}{Fn}{}{}
  \Fn{\upshape \textsc{AttInterp}$(\var{t} \in \vertices)$}{
    $\var{F} \gets \fwd(\var{t}, \allInterpretations)$\;
    $\var{B} \gets \bwd(\var{t}, \allInterpretations)$\;
	\tcc{For every state, compute the BDD difference}
    $\var{NA} \gets \var{F} \setminus \var{B}$\;
    \Return{\upshape $\allInterpretations \setminus \bigcup_{\var{s} \in \vertices(\pbnMath)} \var{NA}(\var{s})$}\;
  }
  \caption{Computation of attractor interpretations $\interpretationSet_A(\var{t})$.}
  \label{algo:attractor-params}
\end{algorithm}
...
\begin{algorithm}
  \SetKwProg{Fn}{Fn}{}{}
  \Fn{\upshape \textsc{StrongBasin}$(\pbnMath{}, \var{t} \in \vertices(\pbnMath), \interpretationSet_A(\var{t}))$}{
    $\var{SB} \gets \bwd(\var{t}, \interpretationSet_A(\var{t}))$\;
    $\var{toUpdate} \gets \{ \var{s} \in \vertices(\pbnMath) \mid \var{SB}(\var{s}) \neq \emptyset \}$\;
    \While{$\var{toUpdate} \neq \emptyset$}{
      $\var{updated} \gets \emptyset$\;
      $\var{toRemove} \gets \forall \var{s} \in \vertices(\pbnMath) \mapsto \emptyset$\;
      \textbf{parallel} \For{$\var{s} \in \var{toUpdate}$}{
        \For{$\var{u} \in \post(\var{s})$}{
          \tcc{Recompute interpretations leading outside of basin}
          $\var{toRemove}(\var{s}) \gets \var{toRemove}(\var{s}) \cup
          \left(
            \var{SB}(\var{s})
            \cap \interpretationSet(\var{s}, \var{u})
            \cap \left(\interpretationSet_A(\var{t}) \setminus \var{SB}(\var{u})\right)
          \right)$\;
        }
        \If{$\var{toRemove}(\var{s}) \neq \emptyset$}{
          $\var{updated} \gets \var{updated} \cup \{\var{s}\}$\;
        }
      }
      $\var{toUpdate} \gets \emptyset$\;
      \For{$\var{s} \in \var{updated}$}{
        \tcc{Update strong basin}
        $\var{SB}(\var{s}) \gets \var{SB}(\var{s}) \setminus \var{toRemove}(\var{s})$\;
        \tcc{Only predecessors of updated states might be updated in the next iteration}
        $\var{toUpdate} \gets \var{toUpdate} \cup \{\var{s}\} \cup \pre(\var{s})$\;
      }
    }
    \Return{\upshape $\var{SB}$}\;
  }
  \caption{Parallel strong basin computation.}
  \label{algo:strong-basin}
\end{algorithm}
...
In complex and highly underspecified models, the empty perturbation
\emptyPerturbation may satisfy the control objective for some
interpretations. This occurs when, under a given interpretation, the source
state already lies in the strong basin of the target, and therefore no
intervention is required. If such interpretations are considered unrealistic,
for example because they contradict the assumption that an intervention is
needed, the set $\interpretationSet_A(t)$ can be replaced by a custom set of
...
\label{ss:symbolic_computation_model}
...
Another important aspect is that a perturbation $\perturbation$ is not a Boolean vector, but rather a vector over $\bools_\unspecified$. For our purposes, however, the algorithms do not need to manipulate whole perturbations. Instead, we only encode which variables in the network are perturbed ($q_i = 0$ or $q_i = 1$) and which are free ($q_i = \unspecified$). For this task, subsets of $\bools^n$ are sufficient. To summarize, we use the following notation:
\begin{itemize}
...
The reason why we do not have to encode full perturbations is because in our algorithms, we use a relation $X \subseteq \vertices \times \allInterpretations \times \mathbb{L}$ to encode the full state of the system. In such an $X$, we assume that if the network is perturbed, only the states that are compatible with the perturbation are permitted (this invariant is maintained throughout the algorithms). Consequently, in a triple $(v, 
...
To evaluate our methods, we have implemented algorithms from the previous section by using internal libraries of the tool AEON~\cite{aeon}. Our experiments were run using a computer with AMD Ryzen Threadripper 2990WX 32-Core Processor and 64GB of memory. We describe the software implementation and how to access in \autoref{chapter:software}. %The repository also includes all auxiliary scripts we used for generating partially specified model versions, running the experiments, and visualizing their results.
...
Next, we conduct experiments evaluating the presented robustness metrics across different types of controls. In particular, we use maximal robustness $\rho_{max}$ and union robustness $\rho_{\cup}$ metrics which we measure on sets of fixed perturbation size. The results are shown in \autoref{fig:maximal_robustness} and \autoref{fig:total_robustness}. 
...
\label{s:discussion}
 ...
 In this section we presented fully symbolic methods for source-target control problem for \PBN{s}. We have described the algorithms for computing one-step, temporary and permanent control relations. Finally, we have evaluated our methods on a benchmark set of real-world biological models with varying sizes and degrees of uncertainty. We have compared the performance of different types of control and also compared our one-step control method to the semi-symbolic approach from \autoref{sec:target_semisymbolic}. We have shown, that new symbolic methods are superior in both performance and scalability, as well as in the size of perturbations enabled by temporary and permanent control.

% In this section, we presented fully symbolic methods for source-target control of partially specified BNs, covering one-step, temporary, and permanent control relations. We evaluated them on real-world biological models with varying sizes and uncertainty levels, comparing both different control types and our one-step method against the semi-symbolic approach from \autoref{sec:target_semisymbolic}. The results show that the symbolic methods improve performance and scalability while enabling smaller perturbation sets through temporary and permanent control.
