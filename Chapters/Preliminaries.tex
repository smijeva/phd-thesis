...
In this section, we summarized the key concepts related to Boolean networks and their dynamics. We introduced the notions of states and updating schemas. We have introduced state space structures, such as trap sets and attractors, which are crucial for understanding the long-term behavior of Boolean networks. Finally, we discussed the relationship between attractors and biological phenotypes, highlighting how these concepts can be used to analyze and interpret the behavior of complex biological systems. Building on these concepts, we now turn our attention to the control of Boolean networks.
...
\item \emph{Phenotype control}: Given a phenotype $\phenotype \subseteq \bools^n$, ensure that from any initial state, the BN converges to an attractor $A \subseteq \allAttractors$ such that $A \subseteq \phenotype$ (\autoref{subfig:st_d}).
\end{enumerate}
...
\subsection{Normalization of partially specified Boolean networks}
...
\subsection{Well-known problems on partially specified Boolean networks}

Now that we have defined PSBNs, we can also consider well-known problems solved for this type of models, in order to understand the challenges they present and give us a better understanding of the tools we can use to solve the control problems.
