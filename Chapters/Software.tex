...
The folder \texttt{src/aeon/} contains the implementation of the high level algorithms taken over from aeon-server package. Since aeon-server implements the algorithms with a focus on the web application, the codebase of the server was not designed to be easily reusable for other purposes. Therefore, we decided to implement a lighweight version of the algorithms for reachability, attractor computation and trap set computation here in the pbn-control library. These algorithms are described in \autoref{subsec:supporting_algos} and \autoref{sec:phen_ctrl_algo}.
